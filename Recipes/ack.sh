 ...
 cp ack-2.12-single-file "$STEWDIR"/Bowls/ack/ack >/dev/null 2>&1
 echo -e "\033[1;34m--> Building $PKGNAME\033[22;0m"
 chmod 0755 "$STEWDIR"/Bowls/ack/bin/ack >/dev/null 2>&1
 ln -s "$STEWDIR"/Bowls/ack/bin/ack /usr/local/bin/ack >/dev/null 2>&1